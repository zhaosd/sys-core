...
import top.zhaosd.core.exception.SysException;
import top.zhaosd.core.service.LocaleMessageService;
import top.zhaosd.core.util.ImageUtil;
...
public class FileController {
    ...
    public R upload(@RequestParam("file") MultipartFile multipartFile)  {
        if (multipartFile.isEmpty() || multipartFile.getOriginalFilename().length() <= 0) {
            throw new SysException("图片不能为空");
        }
        String contentType = multipartFile.getContentType();
        if (!contentType.contains("")) {
            throw new SysException("图片格式错误");
        }
